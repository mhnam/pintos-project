...
void list_swap(struct list_elem *a, struct list_elem *b){
	...
	struct list_elem* a_next;
	struct list_elem* a_prev;
	struct list_elem* b_next;
	struct list_elem* b_prev;

	ASSERT (a != NULL);
	ASSERT (b != NULL);
	if(a==b){ /*no need to swap as list a and list b are equal*/
		return;
	}

	else if(a->next == b){ /*case when elem a and b are sorted sequentially*/
		tmp_prev = a->prev;
		tmp_next = b->next;
...
struct list_elem* nth_elem(struct list* list, size_t n){
